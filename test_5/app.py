...
def comment():
    ...
    comments_db = db.article.find_one({'idx': int(idx)}, {'_id': False})
    comments = list(comments_db['comment'][1:-1].split(','))
    comments.append(comment)

    db.article.update_one({'idx': int(idx)}, {'$set': {'comment': comments}})
